...
export default class Canvas extends cc.Component {
    start () {
        ...
        this.node.on(FINISH_GAME_1, () => {
            this.node.children.forEach(child => child.emit(FINISH_GAME_1));
            // Active Thinking
            this.node.getChildByName('Drawing').active = true;
            this.node.getChildByName('Thinking').active = false;
        });

        this.node.on(FINISH_GAME_2, () => {
            this.node.children.forEach(child => child.emit(FINISH_GAME_2));
            // Active Thinking
            this.node.getChildByName('Drawing').active = true;
            this.node.getChildByName('Thinking').active = false;
        });

        this.node.on(FINISH_GAME_3, () => {
            this.node.children.forEach(child => child.emit(FINISH_GAME_3));
            // Active Thinking
            this.node.getChildByName('Drawing').active = true;
            this.node.getChildByName('Thinking').active = false;
        });
